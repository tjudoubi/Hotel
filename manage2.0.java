...
//						System.out.println("o_index:"+index);
		...
		if(flag == 1){
			...
			int flag_2 = 0;int index_temp = 0;int ff = 0;
			for(int i = 0 ; i < flloor.length;i++){
				Floor floor = flloor[i];
				flag_2 = 0;
				for(int j = 0;j < floor.room.length;j++){
					...
					if(u.number <= room.up){
						flag_2 = 1;
						for(int k = 0;k < room.list.size();k++){
							Inform inform = room.list.get(k);
							if(!(u.in.compareTo(inform.end_date)>0||u.out.compareTo(inform.begin_date)<0)){
								flag_2 = 0;
								break;
							}
							if(k+1 == room.list.size()){
								index_temp = j;
								ff = i;
								break;
							}
			...
			if(flag_2 == 1){
				...
				flloor[ff].room[index_temp].list.add(inform_2);
			}else{
			
				if(u.together == 1&&u.number == 1){
					//System.out.println("together");
					int index_2;int flag_3 = 2;
					for(int i = 0;i < flloor.length;i++){
						for(index_2 = 0; index_2 < 20;index_2++){
							Room r = flloor[i].room[index_2];
							if(r.level == u.level){
								flag_3 = 1;
								for(int j = 0;j < r.list.size();j++){
									Inform inform = r.list.get(j);
